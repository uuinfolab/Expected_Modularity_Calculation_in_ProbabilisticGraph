...
def brute_force(p,c,node,edge):
    ...
    for i in range(2**edge_n):
        ...
        l_permu=bin(i)[2:].zfill(len(edge_possibility))
        ...
        for edge_id in range(len(l_permu)):
            ...
            if int(l_permu[edge_id])==1:
                ...
                p_pw*=edge_possibility[edge_id]
            else:

                p_pw*=(1-edge_possibility[edge_id])
        
        # calculate expected modularity

        Q+=modularity_deter(c,node,pw_lst,edge_position)*p_pw
